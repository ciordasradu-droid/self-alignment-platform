...
const L = {
  ...
  ru: { greet: 'Доброе утро', sleepQ: 'Как спалось?', sleepPh: 'Напиши слово или два…', intentionQ: 'Вчера вечером ты оставил(а) это намерение:', carry: 'Несу его дальше', change: 'Меняю его', changePh: 'Напиши новое намерение…', freshQ: 'Какое намерение ты несёшь с собой сегодня?', freshPh: 'Напиши здесь…', start: 'Начать день', wish: 'Пусть день будет мягким', done: 'Твой день начался.', weekTag: 'Неделя, Увиденная', weekQ1: 'Что ты продолжал(а), даже когда было трудно?', weekQ2: 'Какая закономерность появлялась больше одного раза за эту неделю?', weekQ3: 'Что ты хочешь взять с собой в следующую неделю?', weekPh1: 'Трудные вещи, которые ты продолжал(а) делать…', weekPh2: 'Повторяющиеся закономерности, которые ты заметил(а)…', weekPh3: 'Одно намерение на следующую неделю…' },
}
