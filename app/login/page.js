...
function LoginInner() {
  ...
  // Intrare fara email: cont anonim real (Supabase). Fiecare tester primeste
  // contul LUI — apa lui, ziua lui 1, jurnalul lui. Nu un cont comun.
  // Sesiunea traieste in browserul/aplicatia lui; emailul se poate lega mai tarziu.
  const handleGuest = async () => {
    setStatus('sending')
    const supabase = createSupabaseBrowser()
    const { error } = await supabase.auth.signInAnonymously()
    if (error) {
      console.error('[login:guest]', error.status, error.code, error.message)
      setStatus('guest_off')
      return
    }
    await fetch('/api/try-free', { method: 'POST' }).catch(() => {})
    window.location.href = next
  }

  const errText = status === 'guest_off' ? t('err_guest')
    : status === 'rate' ? t('err_rate')
        ...
        <div style={s.card} className="glass anim-fade-in">
          {status === 'sent' ? (
            ...
            <>
              ...
              {/* TESTARE — cat timp SMTP-ul nu e legat, oricine poate intra fara
                  email: primeste contul lui, apa lui, datele lui. Se scoate
                  punand NEXT_PUBLIC_GUEST_ENTRY=off. */}
              {process.env.NEXT_PUBLIC_GUEST_ENTRY !== 'off' && (
                <>
                  <p style={s.or}>{t('or')}</p>
                  <button onClick={handleGuest} disabled={status === 'sending'} className="pill-btn" style={{ width: '100%' }}>
                    {t('guest')}
                  </button>
                  <p style={s.hint}>{t('guest_note')}</p>
                </>
              )}
            </>
          )}
...
const s = {
  ...
  or: { fontSize:'12px', color:'var(--text-light)', textTransform:'uppercase', letterSpacing:'1px', margin:'20px 0 12px' },
}
